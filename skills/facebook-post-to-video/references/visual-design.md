# Visual design — facebook-post-to-video per-frame shot method
...
This content is competing in a feed, not sitting on a landing page the viewer already chose to visit — the shot rhythm should read faster and denser than a product launch video. Default to **shorter scene windows** (favor 0.8–2s scenes over 2–4s ones), **more frequent internal seams** (`cut-catalog.md`), and a **first scene under ~1s** wherever the hook allows it — the opening beat has to survive a thumb mid-scroll. This is a pacing shift, not a doctrine change: the motion doctrine in `motion-language.md` (smooth `power3`, no lazy breathing, reveal-to-VO, velocity-matched seams) still governs every scene. Denser cuts still hold on the payoff — don't sacrifice the deliberate held read at a frame's resolution just to keep tempo up throughout.
...
- A **deliberately held** frame — content already revealed, now reading still — is legitimate and often right (the punchline, a screenshot the viewer needs a beat to actually read). The failure is never "too still"; it is **front-loaded-then-frozen** (everything dumped by ~25%, nothing cued to the VO). Place held beats deliberately for rhythm so the video isn't uniformly busy (allocate them in `## Video direction`). Reveal pacing + holds + the idle budget → `motion-language.md`.
...
2. **Instantiate its `[slots]` with THIS post's content** — three postures:
   - **Reproduce** — the blueprint fits the beat and your content maps onto its slots cleanly. Fill every `[slot]` with this post's word / asset / stat and follow its Scene timing. Write the resulting Scene lines.
...
The blueprint's own Scene lines, motion vocabulary, and `rule mapping` are your raw material; you are choosing a shape and casting this post's content into it, not copying an engineering spec.
...
```
## Frame 2 — The post, quoted
- scene: the original post's screenshot, held center, caption overlaid   ← refine only if it could read sharper
- voiceover: "…"            ← story's; leave it
- transition_in: crossfade  ← story's; leave it
- type: recap               ← story's
- angle: fact-check
- beat: skepticism
- blueprint: titlecard-reveal (Adapt)   ← you add: the id you instantiated (or "compose")
- focal: assets/post-screenshot.png     ← you add: the hero asset for this beat
- roles: post-screenshot = cutout · logo = supporting · backdrop = background (dim ~40%)  ← you add: role per candidate
- sfx: whoosh-soft                      ← you add: the sound the beat wants (fetched + mounted at root; never yours to embed)

Adapt: keep the titlecard's clean-hold signature; the "title" becomes the real screenshot instead of a typographic card, held long enough to read.
Scene 1 (0.0–1.0s): backdrop dims ~40%; the screenshot slides in from below and settles centered, drop-shadow separating it from the backdrop — Centered template, ~55% of frame. No camera move yet.
Scene 2 (1.0–2.6s): as the VO reads the post's key line aloud, a highlight sweep (`css-marker-patterns`) underlines that exact line inside the screenshot — the viewer's eye is pointed at the same words the VO is saying.
Scene 3 (2.6–3.8s): the screenshot holds fully still and readable — no continuing push, no breathing (a held beat beats bad motion, and a screenshot the viewer can't actually read defeats the point of quoting it).
```
...
- **Framing vocabulary** — centered (hero / climax — good for a single screenshot/meme) · rule-of-thirds · split-screen (comparison, before/after, "them vs. context") · layered-depth (immersive) · asymmetric 60/40 or 70/30 (editorial, room for a reaction line beside the screenshot) · triptych (three panels — good for a listicle beat) · full-width strip. Vary the framing across the video so it doesn't read as one repeated template — let the beat decide, not a quota.
...
- **Don't show**: nav bars, footers, scrollbars, real cursors / browser chrome, generic decorative shapes standing in for a real asset, floating bokeh / purple-blue "AI" gradients — unless it's an intentional UI-demo reconstruction. When a Scene shows the original post's screenshot, keep any third-party UI chrome (platform header, like/comment counts) legible where it matters to the point being made — cropping it out to "clean up" the shot can misrepresent what the post actually said.
...
- **rhythm / held-frame allocation** — name the **held / breather frames** (often the frame quoting the source post, which needs a beat to actually be read) so the video varies its energy: most frames reveal to the VO at native-social tempo, a few hold still (a held read beats bad motion; the anti-monotony discipline; → `motion-language.md`).
...
- **Held frames are deliberate** — allocated in Video direction for rhythm (the source-post frame usually needs one); a held read is fine (prefer stillness to bad motion), but no frame may be front-loaded-then-frozen.
...
- Any Scene showing the source post's screenshot keeps its meaning legible — no crop that changes what the post appears to say.
